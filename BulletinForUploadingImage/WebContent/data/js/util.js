...
function isValidEmail(data)
{
	...
	if (data.search(format) != -1)
		return true;	//-- 적합한 형식 → 사용 가능
	return false;		//-- 부적합 형식 → 사용 불가
}

//○ 01-03. 한글 필터링
function isValidKorean(data)
{
	// UTF-8 코드 중... AC00부터 D7A3 값인지의 여부 검사
	var format = /^[\uac00-\ud7a3]*$/g;
	if (data.search(format) == -1)
		return false;
	return true;	//-- 적합한 형식
...
//○ 02-03. 두 날짜간의 날수를 계산(형식 : 2011-03-01)
function getDays(sDate, eDate)
{
	var date1 = new Date(sDate.split('-')[0], sDate.split('-')[1], sDate
			.split('-')[2]);
	var date2 = new Date(eDate.split('-')[0], eDate.split('-')[1], eDate
			.split('-')[2]);
